...
# =============================================================================
# LE PROMPT VIENT DE `qe_prompt.py`, IL N'EST PLUS ECRIT ICI
# =============================================================================
# Extrait le 13/09/2026, SANS CHANGER UN OCTET -- temoin : l'empreinte du prompt
# sur 36 cas, date neutralisee, identique avant et apres.
#
# POURQUOI : le harnais d'evaluation reconstruisait le prompt de son cote, et
# cinq divergences harnais/production ont ete trouvees en une semaine. Il ne
# pouvait pas importer `app.py` -- son integration continue n'installe que
# `requests` et `fastembed` et exige zero effet de bord au chargement, or
# importer ce fichier-ci leve sans trois secrets, charge deux modeles et appelle
# `get_collections()`. `qe_prompt` n'a besoin que de `pytz`.
#
# ⚠️ LES QUATRE REGLAGES NE SONT PAS IMPORTES ICI, VOLONTAIREMENT.
# POSITIONS_ORALES, ABSTENTION_POSITION, VIVIER_OUVERTURES et
# GRAINE_OUVERTURES sont lus par les fonctions de `qe_prompt`, donc dans LES
# GLOBALES DE `qe_prompt`. Un `from qe_prompt import POSITIONS_ORALES` lierait
# ici une COPIE de la valeur : la basculer n'aurait aucun effet sur le prompt,
# et un test ecrit ainsi ne tomberait pas -- il passerait A VIDE. C'est le pire
# mode de defaillance, et c'est celui qu'un deplacement de fonction cree sans
# rien dire. On les lit donc par `qe_prompt.X`, et un temoin du smoke verifie
# que ce fichier ne porte AUCUN de ces quatre noms.
...
from qe_prompt import (  # noqa: E402
    ...
    _DETAIL_JURIDIQUE,
    _LONGUEUR_MOTS,
    _MAX_TOKENS_PAR_LONGUEUR,
    _MIN_CAR_BLOC_UTILE,
    _MOIS_FR,
    _ORDRE_SACRIFICE,
    _SENTINELLES_VIDE,
    _cle_longueur,
    _resoudre_fichier_ouvertures,
    bloc_porte_quelque_chose,
    blocs_du_critere,
    build_parlementary_response_prompt,
    charger_vivier_ouvertures,
    consignes_abstention,
...
# =====================================================================
# LES DOMAINES GOUVERNEMENTAUX RETENUS — UNE SEULE DÉCLARATION
# ---------------------------------------------------------------------
# ⚠️ Il y en avait DEUX, et elles avaient divergé. Signalé par `feat/eval` le
# 12/09 en préparant la comparaison Google / Tavily, et vérifié par lecture de
# l'arbre syntaxique : Tavily en portait 19, Google 20, et le seul écart était
# `info.gouv.fr` — le site d'information du Gouvernement.
#
# Conséquence concrète, et elle comptait pour la suite : Google Custom Search
# ferme le 01/01/2027, donc une bascule sur Tavily est prévisible. Elle aurait
# fait perdre SILENCIEUSEMENT les résultats `info.gouv.fr`, sans qu'aucun test
# ne tombe — les deux listes étaient justes chacune de son côté.
#
...
def search_tavily_government(subject: str, min_score: float = 0.5):
    ...
    allowed_domains = list(_DOMAINES_GOUV)
